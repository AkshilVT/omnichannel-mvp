...
class YouTubeProcessor extends BaseProcessor {
  ...
  async fetchVideoMetadata(videoId) {
    ...
    try {
      ...
      await this.handleError(error, { videoId });
      throw error;
    }
  }
  ...
  async fetchTranscript(videoId) {
    try {
      ...
      this.logInfo('Transcript not available or failed to fetch', { videoId, error: err.message });
      return null;
    }
  }
   ...
   * @param {string} message - YouTube URL or video ID
  ...
  async process(message) {
    try {
      ...
      // Log the full transcript with new lines and ---
      console.log(
        `\n--- TRANSCRIPT for videoId: ${videoId} ---\n${transcript}\n--- END TRANSCRIPT ---\n`,
      );
